...
export class InMemoryFarmsRepository implements FarmsRepository {
  ...
  async searchManyWithOrders({ cycle_id, page, name }: FarmsRepositorySearchManyWithOrdersRequest): Promise<Farm[]> {
    const orders = await this.inMemomyOrdersRepository.findMany();

    const offers = await this.inMemoryOffersRepository.findMany(cycle_id);

    const offersIdWithOrder = new Set(orders.map((order) => order.offer_id));

    const offersWithOrders = offers.filter((offer) => offer.cycle_id.equals(cycle_id) && offersIdWithOrder.has(offer.id));

    const farmsIdWithOrders = new Set(offersWithOrders.map((offer) => offer.farm_id));
